...
| `committees` | Standing groups of agents: a named, ordered committee a chat can be convened from, the snapshot of its members into `chat_members`, and the provenance column that lets a topic say where its participants came from | S9.1 `[x]` | [context](./features/committees/context.md) · [implement](./features/committees/implement.md) · [frontend](./features/committees/frontend.md) · [backend](./features/committees/backend.md) |
