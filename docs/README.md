...
| `mcp-endpoint` | Witena as an MCP *server*: a stdio shim shipped in the bundle, a local endpoint over the same handler map, discussion tools that return a chat's conclusion to Claude Code, Codex or any MCP client, and Settings → Integrations. In progress; the work packages are in [tasks](./features/mcp-endpoint/tasks.md) | S10.1 `[x]`, S10.2 `[~]`, S10.0 and S10.3–S10.7 `[ ]` | [context](./features/mcp-endpoint/context.md) · [implement](./features/mcp-endpoint/implement.md) · [frontend](./features/mcp-endpoint/frontend.md) · [backend](./features/mcp-endpoint/backend.md) |
