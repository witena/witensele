...
| `e2e/presence.spec.ts` | The real thing: a provider pointing at a loopback listener that accepts the connection and never answers (a non-routable address goes orange then grey, its message is `skipped`, the notice appears, the member on Ollama answers `done`, the run finishes, and Retry against the same dead endpoint leaves it offline. Captures `test-results/shots/presence.png` address was used before, but on some networks the gateway refuses it instantly and the turn ends as a plain error instead of a timeout) |
