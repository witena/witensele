...
| `src/renderer/src/components/agents/agent-picker.tsx` | **S9.2**: the shared "add an agent to this list" popover, extracted from the chat's member panel for the committee editor. It lives here because it is an *agent* list row with the executor rule on it, and it is owned by [`committees`](../committees/frontend.md), which documents why the open state, the outside-click and the positioning stay with the caller |
