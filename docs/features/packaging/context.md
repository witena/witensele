...
| **Updates from the real GitHub feed** | S7.4 built and proved the whole mechanism against a **local generic feed**. The repository is public as of 2026-09-19, which removes what blocked the GitHub provider, but no Release has been published yet, so the real feed has still never been read. The first published, signed Release is the first test of it; recorded in STEPS.md's Phase 6 |
