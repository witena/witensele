...
| **Updates from the real GitHub feed** | S7.4 built and proved the whole mechanism against a **local generic feed**. The repository is public as of 2026-09-19, which removes what blocked the GitHub provider, but no Release has been published yet — the first `v0.1.0` tag failed in packaging because the `CSC_LINK` secret was empty — so the real feed has still never been read. The first published, signed Release is the first test of it; recorded in STEPS.md's Phase 6 |
...
|---|---|---|
| **The release workflow imports the certificate itself; electron-builder never sees `CSC_LINK`** | Let electron-builder import it (its documented CI path); a third-party import action; patch `app-builder-lib` | electron-builder 26.15.3's import cannot work on the runner: it sets the key partition list with the `.p12`'s password instead of the temporary keychain's, and it reads an empty `CSC_LINK` as a path. Eight lines of `security` in the workflow are auditable in place, add no action to pin and trust with a signing identity, and survive an electron-builder upgrade either way; a patch would have to be re-checked at every bump. The identity is then found the same way a local `dist:signed` finds it — auto-discovery over the keychain search list — so CI and the owner's machine take one code path, not two |
| **Nightlies are a tag pushed from the owner's machine; CI builds them; nothing is committed** | Build, sign and notarize on the owner's machine every night; a `schedule:` workflow; bump the patch version on `main` every day | The runner already holds the certificate and the notarization secrets, and does not need a laptop that is awake with its keychain unlocked. A `schedule:` workflow cannot push a tag that triggers `release.yml` (`GITHUB_TOKEN` pushes trigger nothing) without duplicating the release job. A daily bump commit would have to get through `main`'s protection and would push every install to update daily; a `-nightly.<date>` pre-release version needs no commit and lands on its own update channel, which stable installs never read. Drafts stay drafts: a human publishes, so no unreviewed build reaches a user |
