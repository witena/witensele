...
| `src/main/db/settings.test.ts`, `src/main/handlers/handlers.test.ts` (S5.15) | The sandbox setting's path to disk: an `executor` patch stored, kept across a patch of something else and across a reopen, an empty or absent group in a stored row defaulted on read; through the handler, every mode in `EXECUTOR_SANDBOX_MODES` accepted, and a non-object, an unknown key or an unknown mode refused with nothing written |
...
| `src/main/executor/tools.test.ts` (`buildExecutorSection (deliver)`, S5.12, S5.18) | The deliver paragraph asking for the file, its parent folders, the quoted conclusion and the path alone in reply — and no longer claiming the user asked — with the path from `goalHandoffLine` and *without* the implement paragraph; the implement intent keeping its own and not gaining the path-only rule; and all three shapes sharing one prefix, which is what makes the briefing a suffix |
