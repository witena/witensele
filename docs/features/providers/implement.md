...
| `AnthropicCli`, `createAnthropicCli`, `resolveAntBinary`, `antMissing`, `antNotLoggedIn` | `src/main/providers/anthropic-cli.ts` | The CLI seam. `createAnthropicCli` takes `spawn`, `env`, `fallbackDirs` and `now` so the tests drive the real implementation against a fake `ant`, and `bundledDir` — the folder of the `ant` shipped with the app, appended after the fallback directories so an installed CLI wins. `AppContextOptions.bundledAntDir` carries it from `src/main/index.ts`, the only file that knows where the bundle put it |
