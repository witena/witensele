...
| **The app ships `ant` and searches it last** | Show the `brew install` line and wait; download the binary at run time on first sign-in; run the OAuth flow ourselves; let the bundled copy win | Sign in should open a browser, not a terminal. `ant` is MIT and Anthropic publishes checksummed archives, so shipping it is allowed and verifiable; a build-time fetch keeps the binary inside the signature and the notarization, works offline, and has no run-time download to fail. Our own OAuth client is still not on the table (S5.3). The user's own install wins because it is theirs to upgrade and it wrote the profile the bundled copy would otherwise have to read |
