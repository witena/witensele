...
| CLI not installed | For Anthropic this is now rare: the build ships `ant`, so a user of the dmg goes straight from signed out to the browser and never sees this row. It remains for Google, for a checkout that never ran `npm run ant:fetch`, and for the Node server. The same shape plus **that vendor's** install command in monospace. The Sign in button stays clickable on purpose: it doubles as "look again" for a user who installs the CLI in another window |
