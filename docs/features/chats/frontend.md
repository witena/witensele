...
| `document` goal set | The Goal block shows the "Write the deliverable automatically when the discussion concludes" switch, **on** unless this chat's `settings.autoDeliver` is `false` (S5.18). With it on, a discussion that agrees is followed — with nothing clicked — by the `handoffDeliver` line, the executor's `write_file` permission card and the delivered chip; with it off, the conclusion card's own "Write to the deliverable" is the way to get the file |
...
| `chat.viaClient` | The "via {{client}}" badge on a message an IDE sent through the MCP endpoint (S10.4). The **client name itself is never translated** — it is data the client chose, printed as it is stored, the same rule the working directory and the tool names follow |
...
- Each message is an `<article>` carrying `data-sender`, `data-status`,
  ...
  touching copy. S5.18 added `goal-auto-deliver`, a wrapper carrying
  `data-enabled` around the switch, in the shape `mcp-side-effects` and
  `agent-memory-toggle` already use.
