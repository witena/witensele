...
| `invoke('chat.handoff')` | The "Hand to executor" button, through `run.handoff(chatId)`; and the Actions card's "Write the deliverable", through `run.handoff(chatId, 'deliver')` (S5.12) | Stores the hand-off message and runs implement + review ([`orchestration`](../orchestration/frontend.md)). Refused with the same four reasons the controls are disabled for, which the composer's error line then prints |
