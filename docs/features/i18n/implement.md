...
| Namespace | Holds |
|---|---|
| `common` | App name, OK / Cancel / Save / Delete / Add / Search / **Close** (S9.3, the `Dialog` primitive's ×), loading, generic error |
| `nav` | The navigation rail entries; S9.2 added a fourth, `committees` |
| `chat` | Chat list, date groups, member panel, composer, run controls, `passed` / `skipped`, since S5.5 the executor's permission card, the diff block and the file-reference chip, since S5.15 the `commandRisk.*` subtree (one line per `CommandRiskReason`) and the Always allowed block's four keys, and since S9.3 the New chat dialog's eight `newChat*` keys, `committeeBadgeTitle` and the two `syncCommittee*` — all in `chat` rather than in `committees` because what the dialog makes, and what the badge and the button are attached to, is a **chat** |
| `committees` | **S9.2.** The Committees page: the list and its empty state, the editor's fields, the member list (including its own `addMemberAll` / `addMemberEmpty` / `reorderMember` / `moveUp` / `moveDown`), the topics block, the two-step delete, and the `validation.*` subtree. **S9.3** added exactly one, `newTopic` — the button on that page. The strings it deliberately does **not** hold are `chat.executorTaken` and `agents.executorBadge`: the picker and the member row are the same shared components the chat's member panel uses, saying the same rule in the same words, and a second copy of either would be free to drift |
