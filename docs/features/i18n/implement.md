...
| `committees` | **S9.2.** The Committees page: the list and its empty state, the editor's fields, the member list (including its own `addMemberAll` / `addMemberEmpty` / `reorderMember` / `moveUp` / `moveDown`), the topics block, the two-step delete, and the `validation.*` subtree. Two strings it deliberately does **not** hold are `chat.executorTaken` and `agents.executorBadge`: the picker and the member row are the same shared components the chat's member panel uses, saying the same rule in the same words, and a second copy of either would be free to drift |
