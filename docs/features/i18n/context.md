...
| The **updater's own failure message** (S7.4) — a 404 from GitHub, a DNS error. It is interpolated into `settings.about.updates.error` as data, because there is no fixed set of network failures to write copy for; the same call `notices.providerError` already makes | `packaging` / `backend-client` (S7.4) `[x]` |
...
| A flag part may also carry **one value that is data, not copy** (S10.4) | Storing the whole sentence "via claude-code"; a `system-notice` with the client as a parameter | `OriginPart` is `ConclusionPart` with one field: the calling client's own name. The word around it (`chat.viaClient`) is a key, because the user reads it; the name is an identifier the IDE chose for itself, so it is interpolated verbatim, on the same footing as a working directory, a tool name and a model id. A notice would be a second row under the message saying what the message already says, and a stored sentence would freeze "via" in the language that happened to be on screen. |
| A product name is still a **key**, even when both files hold the same string (S10.4) | Literals in the JSX for `Claude Code` and `Codex`; an allowlist entry in `used-keys.test.ts` | `locales.test.ts` already allows a `zh-CN` value that is deliberately identical to English — it is how `Witena` and `English` are handled — so the key costs two lines and keeps the component free of the one exception that invites the next one. It is also what makes adding a third coding agent a locale change rather than a JSX change. |
