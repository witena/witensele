...
| A new `BackendErrorCode` is still right when the failure is about the **machine**, not the request (S5.3's `ant_missing`, `ant_not_logged_in`) | Two more `ValidationReason`s | A reason narrows a refusal of *this request*, and it only ever reaches the screen through `code === 'validation'`. "The Anthropic CLI is not installed" is raised while building a model for a chat turn as well as while validating a form, so it has to travel as a class of its own. Both spellings were used in S5.3: the two `oauth_*` refusals are reasons, the two `ant_*` conditions are codes. |
