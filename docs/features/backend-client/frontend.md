...
| `invoke('integrations.status' / '.connect' / '.disconnect')` | Settings → Integrations, from S10.4's store (WP-12) | The endpoint's switch, and one card per coding agent. All three answer the **same** `IntegrationStatus`, so a click needs no re-read; `connect` also enables `mcpEndpoint`, which is why the section drives its switch from `endpoint.enabled` in that answer rather than from the settings store alone. Both refusals carry a `ValidationReason`, so `translateFailure(t, code, details)` writes the sentence — see [`mcp-endpoint`](../mcp-endpoint/frontend.md) |
