...
| `settings.update` | `{ patch }` | `AppSettings` | Implemented in S1.3; shallow merge, `timeouts`, `editor`, `executor` and `mcpEndpoint` merge per field, unknown keys rejected |
| `integrations.status` | — | `IntegrationStatus` | S10.4. Never rejects for a state: not installed, not connected and endpoint-off are values the Integrations section draws |
| `integrations.connect` | `{ client }` | `IntegrationStatus` | S10.4. Enables `mcpEndpoint` through `settings.update` and registers this build's launcher with the client's own CLI; repairs a registration naming another installation. Refuses with `integrations_no_launcher` / `integrations_client_not_installed` |
| `integrations.disconnect` | `{ client }` | `IntegrationStatus` | S10.4. Idempotent, and leaves the endpoint listening |
...
| `src/main/handlers/integrations.test.ts` | **S10.4**: the three `integrations.*` methods over a stateful fake `IdeClients`, which is what lets the policy be asserted with no coding agent on the machine — and what keeps `npm test` from ever running a real `mcp add`. See [`mcp-endpoint`](../mcp-endpoint/implement.md) |
