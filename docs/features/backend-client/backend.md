...
| `settings.update` | `{ patch: AppSettingsPatch }` | `AppSettings` | `validation` when the patch is not an object or carries a key other than `language`, `theme`, `editor`, `executor`, `timeouts`, `onboardingDismissed`; also for an unknown `theme`, a malformed `editor` or `executor` group (unknown key, an `editor.kind` outside `EDITOR_KINDS`, a blank `editor.command`, an `executor.sandbox` outside `EXECUTOR_SANDBOX_MODES`) or a non-boolean `onboardingDismissed` |
