...
| `src/main/app-context.ts` | `AppContext` (`db`, `repos`, `events`, `secrets`, `userId`, `runners`, `supervisor`, `mcp`, `memory`, `permissions`, `anthropicCli`, `googleCli`, `updates`, `mcpEndpoint`, `mcpLauncherPath`, `ideClients`, `close`) and `createAppContext({ databasePath, userDataDir, secrets, userId?, events?, fetchImpl?, anthropicCli?, googleCli?, runner?, supervisor?, mcp?, updates?, mcpEndpoint?, mcpLauncherPath?, ideClients? })`. `close()` stops every run, the supervisor's loops and every pending permission prompt before closing the database. The last three are S10.3 / S10.4's: the listening MCP host (`null` off the desktop), the absolute path of the `bin/witena-mcp` this build ships (`null` in a checkout) and the coding-agent CLIs — injected for the same reason `anthropicCli` is, so no test ever spawns the developer's own `claude` |
...
| `src/main/handlers/integrations.ts` | S10.4: `integrations.status`, `.connect`, `.disconnect` over `ctx.ideClients` and `ctx.mcpLauncherPath`. It is the one handler module that calls **another handler** — `settings.update`, to enable the endpoint as part of connecting. Owned by [`mcp-endpoint`](../mcp-endpoint/backend.md) |
| `src/main/integrations/ide-clients.ts` | S10.4: the `IdeClients` port and the `node:child_process` implementation — the binary search for `claude` and `codex`, and the six `mcp add` / `get` / `list` / `remove` argument vectors. Electron-free, injectable, and `absentIdeClients()` for every build with no coding agent to reach |
...
| `settings.update` | `{ patch: AppSettingsPatch }` | `AppSettings` | `validation` when the patch is not an object or carries a key other than `language`, `theme`, `editor`, `executor`, `mcpEndpoint`, `timeouts`, `onboardingDismissed`; also for an unknown `theme`, a malformed `editor`, `executor` or `mcpEndpoint` group (unknown key, an `editor.kind` outside `EDITOR_KINDS`, a blank `editor.command`, an `executor.sandbox` outside `EXECUTOR_SANDBOX_MODES`, a non-boolean `mcpEndpoint.enabled`) or a non-boolean `onboardingDismissed`. The **one** handler with a side effect beyond its row: a patch carrying `mcpEndpoint` starts or stops `ctx.mcpEndpoint` after the write, so the endpoint's switch is live (S10.3 — see [`mcp-endpoint`](../mcp-endpoint/backend.md)). A host that refuses is logged rather than raised: the row is the user's intent, and `host.state` is what reports whether a socket came up |
| `integrations.status` | none | `IntegrationStatus` | — . Every state the Integrations section draws is a value: a client that is not installed, one that is not connected, and an endpoint that is switched off (S10.4 — see [`mcp-endpoint`](../mcp-endpoint/backend.md)) |
| `integrations.connect` | `{ client: IdeClientId }` | `IntegrationStatus` | `validation` for an id outside `IDE_CLIENT_IDS`; `validation` with `integrations_no_launcher` in a build that ships no `bin/witena-mcp`, and with `integrations_client_not_installed` when the client's CLI is absent; `internal` quoting the CLI's own `stderr` when it ran and refused. The **second** handler with a side effect beyond its own domain: it calls `settings.update` to enable the endpoint before registering, because an IDE pointed at a closed door is not connected (S10.4) |
| `integrations.disconnect` | `{ client: IdeClientId }` | `IntegrationStatus` | The same two `validation`s; idempotent when nothing is registered. Leaves the endpoint listening — another client may still be using it (S10.4) |
...
| `node:child_process` `execFile` (S10.4) | Running the coding agents' own CLIs for `integrations.*` | Promisified `execFile` rejects on any non-zero exit, and `claude mcp get <name>` exits 1 to say "nothing is registered" — an ordinary state. `ExecFileFn` therefore resolves with `{ code, stdout, stderr }` and rejects only when the binary could not be run or outran its timeout. Commands are argument **vectors**, never shell lines: the launcher's path can contain spaces |
