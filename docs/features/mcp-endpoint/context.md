...
| Decision | Alternatives considered | Why this one |
|---|---|---|
| Registration goes through each client's **own CLI**, never through `~/.claude.json` or `~/.codex/config.toml` | Reading and writing the two files directly, which is two filesystem calls | Those files belong to somebody else. WP-0b measured `codex mcp add` rewriting the whole of `config.toml` and normalising unrelated entries as it went, with the Codex application holding the file open throughout — a third party editing it is a corruption waiting for a race. The CLI is the interface its author supports and the thing that is updated when the format changes |
| Every command is an **argument vector**, never a shell line | Building `claude mcp add witena --scope user -- "$path"` and handing it to a shell | The launcher's path is `<bundle>/Contents/Resources/bin/witena-mcp`, and a bundle can sit in a directory with a space in its name. A vector cannot be re-split, so there is no quoting to get right and no injection to think about — and it is what makes the tests assertable as data |
| `ExecFileFn` **resolves** for a non-zero exit and rejects only for a command that could not be run | Promisified `execFile`, which rejects on any non-zero exit | `claude mcp get witena` exits 1 to say *No MCP server named "witena"*, which is the ordinary "not connected" state the settings section draws. A wrapper that threw there would make the commonest state an exception, and the handler would have to distinguish it from a real failure by parsing the message |
| `connect` **enables the endpoint first**, through the `settings.update` handler | Registering first and switching on afterwards; leaving the switch to the user; writing the settings row directly | An IDE pointed at a closed door is never what the button meant, and registering first leaves a window in which the client exists and its first tool call fails. Going through the handler rather than the repository is what *starts the host* — that side effect is WP-7's and is idempotent, so connecting a second client is free |
| `connect` is also **Repair**: a registration that is not this launcher is unregistered and registered again | A separate `integrations.repair` method; an "overwrite" flag on `register` | `mcp add` over a name that already exists is an error in both CLIs, so a repair cannot be an overwrite anyway. One method means the two paths cannot drift, and WP-12's Repair button is the same call as its Connect |
| `stale` is `false` whenever `launcherPath` is `null` | Calling any registered command stale when there is nothing to compare it with | A development checkout has no stable command, and the user may have registered `node <repo>/out/mcp-shim/witena-mcp.cjs` by hand. Marking that stale would offer a Repair that could only fail with `integrations_no_launcher` |
| `disconnect` leaves the endpoint listening | Switching it off when the last client goes | The endpoint is not owned by the clients: another one, or a hand-written configuration, may still be pointed at it, and PLAN.md makes the switch the user's own gesture for closing the door |
| The two refusals are `ValidationReason`s; a CLI that *ran* and failed is `internal` with its `stderr` quoted | A `BackendErrorCode` per client, as `ant_missing` is; one generic `validation` for all three | The line is the one S5.3 drew: `integrations_no_launcher` and `integrations_client_not_installed` narrow the refusal of *one request* the section sent, which is what a reason is for. A CLI's own complaint is third-party text in whatever language it speaks, so it belongs in the dimmed detail line beside the generic sentence and never in place of it |
| `AppContext.ideClients` defaults to the real implementation in `createAppContext` and to `absentIdeClients()` in `createTestAppContext` | One default in both; a `null` context field like `mcpEndpoint` | Exactly the arrangement `anthropicCli` and `googleCli` already have, and for a stronger reason: a suite that spawned the developer's own `claude` would not only depend on whose machine it ran on, it would write their `~/.claude.json`. `absentIdeClients()` lives beside the real one rather than in `testing.ts` so `src/server/` can use it too |
...
| Decision | Alternatives considered | Why this one |
|---|---|---|
| The switch is driven from `IntegrationStatus.endpoint.enabled`, not from the settings store | Reading `settings.mcpEndpoint.enabled`; re-reading settings after every connect | `integrations.connect` enables the endpoint server-side before it registers anything (WP-11), so a switch bound to the settings row would sit at off after a successful Connect until something else re-read it. The status the section already holds answers both questions and arrives with every call |
| The endpoint line shows **two facts as three states** — off, listening on a port, on-but-not-listening | One "on / off" pill following the switch | WP-7 lets a host that fails to start leave the row `true`, and that is exactly the state a user reports. A pill that agreed with the switch would tell them nothing in the one case they need it |
| The toggle writes **through the settings store** (`setMcpEndpoint`) and then re-reads `integrations.status` | Calling `settings.update` straight from the integrations store; holding a second copy of the row | The settings store is the app-wide mirror of that row and the write is what starts and stops the host. Two mirrors that disagree are worse than one extra read, and `listening` is only knowable after the write has landed |
| A card per entry of `clients`, in the order it arrives, with no id matched in the renderer | A component per client; a `Record<IdeClientId, …>` of cards | WP-11 guarantees the list is `IDE_CLIENT_IDS`, always full and always in order, so a third client becomes a card with no renderer change. The only per-client thing left is its name, which is a locale key |
| Repair is drawn as its own state but calls `integrations.connect` | A separate method; a confirm dialog before repairing | It is WP-11's rule, and the reason the two cannot drift. What changes between Connect and Repair is the sentence and the button's word, which is a display concern and lives in `integration-display.ts` |
