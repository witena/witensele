...
| File | Owns |
|---|---|
| `schema.ts` | The drizzle definition of the eight tables and the `…Row` types inferred from them |
...
| File | Covers |
|---|---|
| `src/main/db/migrations.test.ts` | Migrations are bundled and split on the breakpoint marker; opening a file creates all eight tables plus `__migrations` (S5.15 added `permission_grants` to that list); each migration is recorded once and a second run applies nothing; closing, reopening and re-running keeps both the schema and the data; WAL and `foreign_keys` are on; `':memory:'` works |
