...
| File | Owns |
|---|---|
| `schema.ts` | The drizzle definition of the ten tables and the `…Row` types inferred from them |
...
| `postgres/schema.ts` | **S8.1**: the same ten tables in `drizzle-orm/pg-core` |
...
| File | Covers |
|---|---|
| `src/main/db/migrations.test.ts` | Migrations are bundled and split on the breakpoint marker; opening a file creates all ten tables plus `__migrations` (S5.15 added `permission_grants` to that list, S9.1 `committees` and `committee_members`); each migration is recorded once and a second run applies nothing; closing, reopening and re-running keeps both the schema and the data; WAL and `foreign_keys` are on; `':memory:'` works |
...
| `src/main/db/settings.test.ts` | Defaults when nothing is stored; shallow merge persisted across a reopen; `timeouts` merged field by field (including S5.15's `permissionTimeoutMs`); a stored row missing newer fields is filled in from the defaults; one row per user |
| `src/main/db/postgres/schema-drift.test.ts` | **S8.1**: the SQLite and Postgres schemas declare the same ten tables, and each table the same columns, nullability, defaults, primary keys and enum values. Needs no database, so it runs everywhere; includes a guard on itself (a table with a column removed must compare unequal) |
...
| `src/main/db/permissionGrants.test.ts` (S5.15) | Empty for a chat nobody granted anything in; the point read `has` used on every gated call; scoping to one chat; surviving a reopen, which is the whole reason the table exists; a repeat grant keeping its first timestamp; newest-first ordering; revoking one and only one; an idempotent revoke; and the cascade read straight out of the table |
...
- **`chat_members` and `permission_grants` have no `userId`** — both are pure
  join tables, scoped through the chat (and, for the first, the agent), which is
  checked before a write. A cascade cannot check a second column anyway.
