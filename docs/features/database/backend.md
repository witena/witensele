...
| File | Responsibility |
|---|---|
| `src/main/db/schema.ts` | drizzle definitions of the ten tables plus the inferred `…Row` types |
...
| `src/main/db/migrations/0004_messy_the_renegades.sql` | S5.15: `permission_grants` |
| `src/main/db/migrations/0005_ambiguous_sersi.sql` | S9.1: `committees`, `committee_members`, `chats.committee_id` |
...
| `src/main/db/repositories/committees.ts` | **S9.1**: committee CRUD with its ordered membership written in the same transaction |
| `src/main/db/repositories/chats.ts` | Chat CRUD, `setMembers`, `listMembers`, `listChatIdsForAgent`, `listChatIdsForCommittee` (S9.1), `DEFAULT_CHAT_TITLE` |
...
| `src/main/db/postgres/schema.ts` | **S8.1**: the same ten tables in `drizzle-orm/pg-core` — `bigint` timestamps, `boolean`, `jsonb` |
...
| `src/main/db/postgres/migrations/0001_permission_grants.sql` | **S5.15**: `permission_grants`, hand-written |
| `src/main/db/postgres/migrations/0002_committees.sql` | **S9.1**: `committees`, `committee_members`, `chats.committee_id`, hand-written |
...
| `committee_id` | text null | **S9.1**: the committee this topic was convened from, or null. FK → `committees.id`, **`ON DELETE SET NULL`** — provenance, not membership, so deleting the committee clears the column and leaves `chat_members` alone. Null is what every row written before S9.1 holds |
...
|---|---|---|
| `id` / `user_id` / `created_at` / `updated_at` | | As above |
| `name` | text not null | Trimmed, non-empty, at most `MAX_COMMITTEE_NAME_CHARS` (100). Not unique: a committee name is a label, not an identity — nothing resolves `@Review` |
| `description` | text not null default `''` | |

### `committee_members` (S9.1)

| Column | Type | Notes |
|---|---|---|
| `committee_id` | text not null | FK → `committees.id`, `ON DELETE CASCADE` |
| `agent_id` | text not null | FK → `agents.id`, `ON DELETE CASCADE` |
| `position` | integer not null | Speaking order, 0-based, the index in `Committee.memberAgentIds` |

Composite primary key `(committee_id, agent_id)`, shaped exactly like
`chat_members`. The agent cascade leaves a **gap** in the positions; members are
read `ORDER BY position` into an array, so the gap closes on read and the next
write renumbers from 0. See [`committees`](../committees/backend.md).
...
| `0005_ambiguous_sersi.sql` | S9.1 | `CREATE TABLE committees`, `CREATE TABLE committee_members`, `ALTER TABLE chats ADD committee_id`. **Hand-edited after generation**: drizzle-kit emits the new column's `REFERENCES` clause without the `ON DELETE set null` its own snapshot records — and without the action a committee delete is refused by the foreign key instead of clearing the column — and it ordered the two `CREATE TABLE`s alphabetically. Both fixes are explained in the file's header |
...
| `postgres/migrations/0001_permission_grants.sql` | S5.15 | `permission_grants`, hand-written: the first table added after the Postgres dialect existed |
| `postgres/migrations/0002_committees.sql` | S9.1 | `committees`, `committee_members` and `chats.committee_id`, hand-written in the same shape SQLite's `0005` creates. The column is added and its constraint attached in separate statements so the file is re-runnable |
